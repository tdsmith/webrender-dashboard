...
slow_content_paint_time_summary = tbl(cache, "slow_content_paint_time_summary") %>% collect
slow_content_full_paint_time_summary = tbl(cache, "slow_content_full_paint_time_summary") %>% collect
slow_composite_time_summary = tbl(cache, "slow_composite_time_summary") %>% collect
...
all_builds = perf_medians %>% distinct(normalized_channel, app_build_id) %>% ungroup
recent_builds = bind_rows(
  all_builds %>% filter(normalized_channel == "nightly") %>% top_n(14, app_build_id),
  all_builds %>% filter(normalized_channel == "beta") %>% top_n(5, app_build_id)
)
```
...
all_builds = crashes_per_build %>% distinct(normalized_channel, app_build_id) %>% arrange(normalized_channel, app_build_id) %>% pull(app_build_id)
cr_labels = setNames(rep("", length(all_builds)), all_builds)
label_idx = rep_len(c(TRUE, FALSE, FALSE, FALSE), length(all_builds))
cr_labels[label_idx] = all_builds[label_idx]
