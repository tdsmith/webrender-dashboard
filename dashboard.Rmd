...
cache = DBI::dbConnect(SQLite(), dbname="webrender.sqlite3")

crashes_per_build = tbl(cache, "crashes_per_build") %>% collect
crashes_per_build_per_process = tbl(cache, "crashes_per_build_per_process") %>% collect
perf_medians = tbl(cache, "perf_medians") %>% collect
paint_time_quantiles = tbl(cache, "paint_time_quantiles") %>% collect
slow_content_frame_time_summary = tbl(cache, "slow_content_frame_time_summary") %>% collect
...
**P1**: Blocking beta; **P2**: Blocking release (in progress/looking for owners); **P3**: Blocking release (in queue)
...
```{r recent_nightly_performance}
quotient_seq = function(df) {
  enabled = filter(df, branch == "enabled")
  disabled = filter(df, branch == "disabled")
  ratio = enabled$user_median/disabled$user_median
  regression = enabled$log_ymin > disabled$log_ymax
  data.frame(percent_gecko=ratio*100, regression=regression)
}

content_frame_time_exceeded = function(build_id) {
  df = slow_content_frame_time_summary %>%
    filter(app_build_id == build_id)
  enabled = filter(df, branch == "enabled")
  disabled = filter(df, branch == "disabled")
  ratio = enabled$y/disabled$y
  regression = enabled$ymin > disabled$ymax
  data.frame(percent_gecko=ratio*100, regression=regression, metric="Per-user fraction CONTENT_FRAME_TIME > 200% vsync")
}

perf_medians %>%
  filter(normalized_channel == "nightly", app_build_id == last_build_id, metric != "composite_time", metric != "content_frame_time") %>%
  group_by(metric) %>%
  do(quotient_seq(.)) %>%
  bind_rows(content_frame_time_exceeded(last_build_id)) %>%
  arrange(desc(percent_gecko)) %>%
  mutate(percent_gecko=sprintf("%.1f%%", percent_gecko))
```
...
perf_medians %>%
  filter(normalized_channel == "beta", app_build_id == last_beta_build, metric != "composite_time", metric != "content_frame_time") %>%
  group_by(metric) %>%
  do(quotient_seq(.)) %>%
  bind_rows(content_frame_time_exceeded(last_beta_build)) %>%
  arrange(desc(percent_gecko)) %>%
  mutate(percent_gecko=sprintf("%.1f%%", percent_gecko))
```
...
```{r total_crash_count}
ggplot(crashes_per_build, aes(app_build_id, total_crashes, fill=branch)) +
...
```{r total_crash_rate}
ggplot(crashes_per_build, aes(app_build_id, total_crashes/usage_hours*1000, fill=branch, alpha=usage_hours)) +
  facet_grid(~normalized_channel, space="free_x", scales="free_x", drop=TRUE) +
  geom_col(position=position_dodge()) +
...
```{r device_reset_rate}
ggplot(crashes_per_build, aes(app_build_id, device_reset_reason_total/usage_hours*1000, fill=branch, alpha=usage_hours)) +
  facet_grid(~normalized_channel, space="free_x", scales="free_x", drop=TRUE) +
  geom_col(position=position_dodge()) +
  geom_errorbar(
    mapping=aes(
...
crashes_per_build_per_process %>%
  ...
  ggplot(aes(app_build_id, total_crashes/usage_hours*1000, fill=branch, alpha=usage_hours)) +
    ...
    geom_errorbar(position=position_dodge(w=1), width=0.2, mapping=aes(ymin=ymin/usage_hours*1000, ymax=ymax/usage_hours*1000)) +
    scale_alpha_continuous(trans="log10", range=c(0.25, 1)) +
    coord_cartesian(ylim=c(0, 0.5)*1000) +
    theme(axis.text.x=element_text(angle=45, hjust=1)) +
...
```{r shutdown_crashes}
ggplot(crashes_per_build, aes(app_build_id, shutdown_crashes, fill=branch, alpha=usage_hours)) +
  facet_grid(~normalized_channel, space="free_x", scales="free_x", drop=TRUE) +
  geom_col(position=position_dodge()) +
  scale_alpha_continuous(trans="log10", range=c(0.25, 1)) +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
...
```{r oom_crashes}
ggplot(crashes_per_build, aes(app_build_id, oom_crashes, fill=branch, alpha=usage_hours)) +
  facet_grid(~normalized_channel, space="free_x", scales="free_x", drop=TRUE) +
  geom_col(position=position_dodge()) +
  scale_alpha_continuous(trans="log10", range=c(0.25, 1)) +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
...
perf_medians %>%
  ...
  ggplot(aes(app_build_id, user_median, ymin=10**log_ymin, ymax=10**log_ymax, fill=branch)) +
    ...
    labs(title="CONTENT_FRAME_TIME [% vsync]: per-user mean", x="Build ID", y="Median of per-user means")
```
...
ggplot(slow_content_frame_time_summary, aes(app_build_id, y, ymin=ymin, ymax=ymax, fill=branch)) +
  facet_grid(~normalized_channel, scales="free_x", space="free_x") +
  geom_col(position=position_dodge()) +
  geom_errorbar(width=0.2, position=position_dodge(width=1)) +
  theme(axis.text.x=element_text(angle=30, hjust=1)) +
...
```{r intensity}
ggplot(intensity_summary, aes(app_build_id, y, ymin=ymin, ymax=ymax, fill=branch)) +
  facet_grid(~normalized_channel, scales="free_x", space="free_x") +
  geom_col(position=position_dodge()) +
  geom_errorbar(width=0.2, position=position_dodge(width=1)) +
  theme(axis.text.x=element_text(angle=30, hjust=1)) +
...
```{r uri_active_hour}
ggplot(uri_summary, aes(app_build_id, user_median, ymin=10**log_ymin, ymax=10**log_ymax, fill=branch)) +
  facet_grid(~normalized_channel, scales="free_x", space="free_x") +
  geom_col(position=position_dodge()) +
  geom_errorbar(width=0.2, position=position_dodge(width=1)) +
  theme(axis.text.x=element_text(angle=30, hjust=1)) +
...
P3 = Blocking release (pending queue)
...
burndown %>%
  ...
  ggplot(aes(Day, Count)) +
    ...
    expand_limits(y=0)
```
[Bugzilla: P1 blockers](https://bugzilla.mozilla.org/buglist.cgi?list_id=14424815&o1=substring&v1=1386669&priority=P1&f1=blocked&resolution=---&query_format=advanced)
...
[Bugzilla: P2 blockers](https://bugzilla.mozilla.org/buglist.cgi?list_id=14424815&o1=substring&v1=1386669&priority=P2&f1=blocked&resolution=---&query_format=advanced)
...
[Bugzilla: P3 blockers](https://bugzilla.mozilla.org/buglist.cgi?list_id=14424815&o1=substring&v1=1386669&priority=P3&f1=blocked&resolution=---&query_format=advanced)
...
ggplot(enrollment_summary, aes(as.Date(submission_date_s3, "%Y%m%d"), n, color=branch)) +
  facet_grid(normalized_channel~., scales="free_y") +
  geom_point() +
...
unenrollment_summary %>%
  ...
  ggplot(aes(as.Date(submission_date_s3, "%Y%m%d"), n, fill=reason)) +
    facet_grid(normalized_channel~branch, scales="free_y") +
    geom_col() +
    theme(legend.position="bottom") +
...
This notebook follows users enrolled in the experiment `prefflip-webrender-v1-2-1492568`.

Data are collected from Spark with [this Databricks notebook](https://dbc-caf9527b-e073.cloud.databricks.com/#notebook/45053/command/45057).
